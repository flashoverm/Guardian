...
function insert_event($date, $start, $end, $location, $type, $title, $comment, $manager){
	global $db;

	$uuid = getGUID();
	$hash = hash("sha256", $uuid.$date.$start.$end.$location.$type.$title);
	$query = "INSERT INTO events (uuid, date, start_time, end_time, location, type, title, comment, hash, manager)
		VALUES ('".$uuid."', '".$date."', '".$start."', '".$end."', '".$location."', '".$type."', '".$title."', '".$comment."', '".$hash."', '".$manager."')";
